...
# Default pretrain checkpoints (can be overridden via environment variables)
DEFAULT_PRETRAIN_RESNET10_PATH = os.environ.get(
    "PRETRAIN_RESNET10_PATH", "./pretrain/resnet_10_23dataset.pth"
)
DEFAULT_PRETRAIN_RESNET18_PATH = os.environ.get(
    "PRETRAIN_RESNET18_PATH", "./pretrain/resnet_18_23dataset.pth"
)
...
class Medical_3DNet(nn.Module):
    ...
    def __init__(self, classifier_OCT_dims,num_classes=10):
        ...
        # ---- ResNet Backbone ----
        self.resnet_3DNet = generate_model(
            model_type='resnet',
            model_depth=10,
            input_W=classifier_OCT_dims[0][0],
            input_H=classifier_OCT_dims[0][1],
            input_D=classifier_OCT_dims[0][2],
            resnet_shortcut='B',
            no_cuda=True,
            gpu_id=[0],
            pretrain_path=DEFAULT_PRETRAIN_RESNET10_PATH,
...
class Medical_base2_3DNet(nn.Module):
    ...
    def __init__(self, classifier_OCT_dims,num_classes=10):
        ...
        self.resnet_3DNet = generate_model(
            ...
            no_cuda=True,
            gpu_id=[0],
            pretrain_path=DEFAULT_PRETRAIN_RESNET18_PATH,
            nb_class=num_classes,
        )
...
class Multi_ResNet(nn.Module):
    ...
    def forward(self, X, y, training=True):
        ...
        if training:
            grouped_fudus_feature = self.group_features_by_label(
                y.cpu(), backboneout_1.cpu().detach().numpy()
            )
            grouped_oct_feature = self.group_features_by_label(
                y.cpu(), backboneout_2.cpu().detach().numpy()
            )
            labels = sorted(grouped_fudus_feature.keys())
            T_dict, log = get_coupling_egw_labels_ott(
                (grouped_fudus_feature, grouped_oct_feature)
            )

            device = backboneout_1.device
            OT_fudus = torch.from_numpy(
                np.concatenate([grouped_fudus_feature[l] for l in labels])
            ).to(device)
            OT_oct = torch.from_numpy(
                np.concatenate([grouped_oct_feature[l] for l in labels])
            ).to(device)

            T_np = self.mdict_to_matrix(
                T_dict,
                np.concatenate(
                    [np.ones(grouped_fudus_feature[l].shape[0]) * l for l in labels]
                ),
                np.concatenate(
                    [np.ones(grouped_oct_feature[l].shape[0]) * l for l in labels]
                ),
            )
            T = torch.from_numpy(T_np).to(device)
            T[T.sum(dim=-1) == 0, :] = 1e-8

            x_idx = torch.arange(OT_fudus.shape[0], device=device)
            ot_loss = 0.0
            pred_oct_feature = []
            for i in range(OT_fudus.shape[0]):
            ...
            ot_loss /= OT_fudus.shape[0]
            pred_oct_feature = torch.stack(pred_oct_feature)
            backboneout_2 = pred_oct_feature
        ...
        # bs,8192
        if training:
            # backboneout_2 = self.oct_fusion(torch.cat([backboneout_2,pred_oct_feature],1))
...
class MLC(nn.Module):
    def __init__(self, classes, modalties, classifiers_dims, lambda_epochs=1):
        ...
        self.res2net_2DNet = Medical_base_2DNet(num_classes=self.classes)
        self.intermediate_dim_2D = 2048

        classifier_OCT_dims = classifiers_dims[0]
        self.resnet_3DNet = Medical_base_3DNet(classifier_OCT_dims, num_classes=self.classes)
        self.intermediate_dim_3D = 4096
